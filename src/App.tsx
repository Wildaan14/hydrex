...
const ErrorFallback = ({ error, resetErrorBoundary }: any) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900 p-6 text-center w-full text-foreground">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100 dark:border-red-900">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-6 text-sm">
          {isChunkError
            ? "A new version of the app was deployed. Please reload the page to get the latest features."
        ...
        <button
          ...
          className="w-full py-3 px-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-medium transition-colors focus:ring-4 focus:ring-emerald-500/20 shadow-lg shadow-emerald-500/20"
        >
          {isChunkError ? "Reload to update" : "Try again"}
        </button>
      </div>
    </div>
  );
...
function App() {
  React.useEffect(() => {
    // Handle dynamic import failures (usually due to redeployments)
    const handleChunkError = (event: PromiseRejectionEvent) => {
      ...
      if (
        ...
        console.warn("Chunk loading error detected. Forcing page reload...");
